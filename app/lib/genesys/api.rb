...
module Genesys
  class Api
    ...
    def notification_subscriptions # rubocop:disable Metrics/MethodLength
      topics = [
        {
          'id' => "v2.workforcemanagement.businessunits.#{business_unit_id}.schedules",
          'state' => 'Permitted'
        }
      ]
    ...
    def signed_upload(schedule, schedule_payload)
      path = schedule.published_schedule_uri.dup.concat('/update/uploadurl')
      payload = { 'contentLengthBytes' => schedule_payload.to_json.bytesize }

      response = token.post(
        path, body: payload.to_json, headers: { 'Content-Type' => 'application/json' }, snaky: false
      )
    ...
    def business_unit_id
      ENV.fetch('GENESYS_BUSINESS_UNIT_ID', '71876e55-8410-4ac4-b675-ec6724cde3ec')
    end
  end
end
