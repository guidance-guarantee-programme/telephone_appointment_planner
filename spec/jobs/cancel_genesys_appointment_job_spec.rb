...
RSpec.describe CancelGenesysAppointmentJob, '#perform' do
  context 'when the appointment is cancellable' do
    let(:appointment) { double(:appointment, cancel_to_genesys?: true) }
  ...
  context 'when the appointment is not pushable' do
    it 'does not delegate to the genesys pusher' do
      appointment = double(:appointment, cancel_to_genesys?: false)
